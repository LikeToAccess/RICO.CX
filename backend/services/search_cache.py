...
class SearchCache:
	"""
	Thread-safe in-memory cache for indexer search results with TTL expiration.
	"""
	...
	def _clean_expired_under_lock(self) -> None:
		now = time.time()
	...
	def get_by_matching_cards(self, query: str, category: Optional[str]) -> List[AggregatedResult]:
		with self.lock:
			...
			matches: List[AggregatedResult] = []
			for (title_lower, _year, is_tv), (card, _expiry) in list(self._cards_cache.items()):
	...
	def set(self, query: str, category: Optional[str], results: List[AggregatedResult]) -> None:
		with self.lock:
			self._clean_expired_under_lock()
			now = time.time()
			expiry = now + self.ttl
